...
function AddTenant({setError}){
 ...
 async function submit(e){
  ...
  const body=Object.fromEntries(data);body.weekly_rent=Number(body.weekly_rent||0);body.date_of_birth=body.date_of_birth||null;
  setError("");try{
...
function Payments({user,setError,reloadUser}){
 ...
 <section><h2>{mode==="admin"?"Payment submissions":"My payment submissions"}</h2>{!items.length?<p className="muted">No payments submitted yet.</p>:<table><thead><tr>{mode==="admin"&&<th>Tenant</th>}<th>Amount</th><th>Date</th><th>Bank reference</th><th>Details</th><th>Receipt</th><th>Status</th><th>Action</th></tr></thead><tbody>{items.map(p=><tr key={p.id}>{mode==="admin"&&<td>{p.tenant_name} (#{p.tenant_id})</td>}<td>{aud(p.amount)}</td><td>{p.payment_date}</td><td>{p.bank_reference}</td><td>{p.bank_details}</td><td>{mode==="admin"&&p.has_receipt?<button className="link" onClick={()=>receipt(p)}>View image</button>:p.has_receipt?"Attached":"—"}</td><td>{p.state}{p.review_note&&<small> — {p.review_note}</small>}</td><td>{mode==="admin"&&p.state==="pending"?<div className="actions"><button disabled={busy} onClick={()=>review(p,"approved")}>Approve</button><button disabled={busy} onClick={()=>review(p,"rejected")}>Reject</button></div>:"—"}</td></tr>)}</tbody></table>}</section></>
}
